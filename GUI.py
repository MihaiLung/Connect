...
class setup_window:
    ...
    def __init__(self, master):
        ...
        self.p1_Vision_down = Button(self.frame_vision_1, image=self.DownPhoto,
                                     command=lambda: self.update_player_vision(1, -1), height=self.button_height,
                                     width=self.button_width)
        self.p1_Vision_up = Button(self.frame_vision_1, image=self.UpPhoto,
                                   command=lambda: self.update_player_vision(1, 1), height=self.button_height,
                                   width=self.button_width)
        self.p2_Vision_down = Button(self.frame_vision_2, image=self.DownPhoto,
                                     command=lambda: self.update_player_vision(2, -1), height=self.button_height,
                                     width=self.button_width)
        self.p2_Vision_up = Button(self.frame_vision_2, image=self.UpPhoto,
                                   command=lambda: self.update_player_vision(2, 1), height=self.button_height,
                                   width=self.button_width)
        ...
        self.dummy_label_3 = Label(self.frame_player_setting, text="      ")
        self.dummy_label_3.grid(row=5, column=0, columnspan=2)

        self.frame_vision_1.grid(row=6, column=0)
        self.frame_vision_2.grid(row=6, column=1)

        self.dummy_label_3 = Label(self.frame_player_setting, text="      ")
        self.dummy_label_3.grid(row=7, column=0, columnspan=2)
        ...
        self.p1_human = Radiobutton(self.frame_player_setting, text="Human", variable=self.p1human, value=1,
                                    command=lambda: self.toggle_view(1, 0))
        self.p1_AI = Radiobutton(self.frame_player_setting, text="AI", variable=self.p1human, value=0,
                                 command=lambda: self.toggle_view(1, 1))
        self.p1_AI.grid(sticky="W", row=2, column=0)
        self.p1_human.grid(sticky="W", row=3, column=0)
        self.p2_AI = Radiobutton(self.frame_player_setting, text="AI", variable=self.p2human, value=0,
                                 command=lambda: self.toggle_view(2, 1))
        self.p2_human = Radiobutton(self.frame_player_setting, text="Human", variable=self.p2human, value=1,
                                    command=lambda: self.toggle_view(2, 0))
        ...
        self.player1_LTO_slider = Scale(self.player1_frame_slider, from_=0, to=100, orient=HORIZONTAL,
                                        variable=self.player1_LTO)
        ...
        self.player2_LTO_slider = Scale(self.player2_frame_slider, from_=0, to=100, orient=HORIZONTAL,
                                        variable=self.player2_LTO)
        ...
        self.sliders_explanation = Message(self.frame_sliders,
                                           text="Lower long term orientation means less weight is put on outcomes further away in the future compared to ones closer to the player's current state.",
                                           width=300)
        ...
        self.dummy_label = Label(self.frame_end_buttons, text="     ")
        self.dummy_label.grid(row=0)
        self.validate_button = Button(self.frame_end_buttons, text="Validate", command=self.validate, height=1,
                                      width=10)
    ...
    def validate(self):
        ...
        if numeric:
            for input in [self.rows_label_input.get(), self.cols_label_input.get(), self.connects_label_input.get(),
                          self.player1sims_input.get(), self.player2sims_input.get()]:
                try:
                    int(input)
                except ValueError:
                    c += 1
        ...
        elif int(self.rows_label_input.get()) > 10 and int(self.cols_label_input.get()) + int(
                self.rows_label_input.get()) >= 15:
            c += 1
        ...
        elif int(self.cols_label_input.get()) > 10 and int(self.cols_label_input.get()) + int(
                self.rows_label_input.get()) >= 15:
            c += 1
        ...
        if max([int(self.cols_label_input.get()), int(self.rows_label_input.get())]) <= int(
                self.connects_label_input.get()):
            pass
    ...
    def initialise_game(self, game):
        ...
        for i in range(self.rows):
            ...
            for j in range(self.columns):
                rect = pg.Rect(self.margin + j * (self.margin + self.edge), self.margin + i * (self.margin + self.edge),
                               self.edge, self.edge)
        ...
        for i in range(self.columns):
            highlight_rect = pg.Rect(self.margin + i * (self.margin + self.edge) - self.margin / 2, self.margin,
                                     self.edge + self.margin, self.height)
